...
MIN_HIST      = 1.0    # somme minimum histogramme (masque float32)
MIN_WIN       = 0.35   # somme minimum fenêtre
...
HOLD_FRAMES   = 8      # frames de maintien — plus stable en quality medium
BASE_SEARCH_W = 0.13   # demi-largeur recherche autour position précédente (ratio image)
MAX_DEV_JUMP  = 0.050  # saut max déviation par frame — plus strict
BASE_EMA      = 0.12   # inertie forte sur les bases
POLY_SMOOTH   = 0.15   # lissage EMA des coefficients polynomiaux (zone verte)
MAX_BASE_JUMP = 0.10   # saut max absolu d'une base par frame (ratio image)

# Seuils couleur HLS pour CARLA (environnement synthétique)
#  Jaune : ligne centrale — TOUJOURS à gauche de notre voie
HLS_YELLOW_LO = np.array([15,  50,  70], dtype=np.uint8)
HLS_YELLOW_HI = np.array([40, 230, 255], dtype=np.uint8)
#  Blanc : lignes de bord de voie (droit)
HLS_WHITE_LO  = np.array([ 0, 150,   0], dtype=np.uint8)
HLS_WHITE_HI  = np.array([180, 255,  90], dtype=np.uint8)
...
class LaneDetectionNode:
    ...
    def _cb_cmd(self, msg):
        with self._lock:
            ...
            # Quand TURN_RIGHT commence, réinitialiser l'état de tracking
            # pour détecter la nouvelle voie proprement après le virage
            if msg.data == 'TURN_RIGHT' and prev != 'TURN_RIGHT':
                self._prev_lx    = None
                self._prev_rx    = None
                self._lw_px      = None
                self._dev_prev   = 0.0
    ...
    def _find_bases(self, yellow_f, white_f):
        ...
        # Filtre saut maximal : rejette les lignes transversales d'intersection
        # MAX_BASE_JUMP était défini mais jamais appliqué — c'est ici qu'il agit
        if self._prev_lx is not None and lb is not None:
            if abs(lb - self._prev_lx) / w > MAX_BASE_JUMP:
                lb = self._prev_lx   # ignorer le saut → garder position précédente
        if self._prev_rx is not None and rb is not None:
            if abs(rb - self._prev_rx) / w > MAX_BASE_JUMP:
                rb = self._prev_rx
    ...
    def _deviation(self, lpts, rpts, w):
        ...
        if valid:
            ...
            if self._no_det_cnt <= HOLD_FRAMES:
                # Décroissance vers 0 : évite de propager un biais de virage à l'intersection
                decay = max(0.0, 1.0 - self._no_det_cnt / HOLD_FRAMES)
                return prev * decay
            dev_raw = 0.0
